...
from pooch import check_version
...
class IDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        z = []  # inference output
        self.training |= self.export
        for i in range(self.nl): #遍历不同尺度的检测层(3)
...
class IDetect_Decoupled_Att(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export
    end2end = False
    include_nms = False
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        super(IDetect_Decoupled_Att, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
        ...
        self.m_reg_conf = nn.ModuleList(
            nn.Sequential(SpatialAttention(x),Conv(x,x,3))for x in ch)  # reg_conf stem conv
        self.m_reg = nn.ModuleList(
            nn.Sequential(nn.Conv2d(x, self.na * 4, 1)) for x in ch)  # reg conv
...
#         self.m_stem = nn.ModuleList(Conv(x, x, 1) for x in ch)  # stem conv
#         self.m_cls = nn.ModuleList(
#             nn.Sequential(CA_Layer(x),Conv(x, x, 3),nn.Conv2d(x, self.na * self.nc,1)) for x in ch)  # cls conv
#         # self.m_cls = nn.ModuleList(
#         #     nn.Sequential(Conv(x,256,1),CA_Layer(256), Conv(256, 256, 3),Conv(256, 256, 3),Conv(256,x,1),nn.Conv2d(x, self.na * self.nc, 1)) for x in ch)  # cls conv
#
#         self.m_reg_conf = nn.ModuleList(
#             nn.Sequential(My_SA(),Conv(x, x, 3))for x in ch)  # reg_conf stem conv
#         # self.m_reg_conf = nn.ModuleList(nn.Sequential(Conv(x,256,1),SA_Layer(256), Conv(256, 256, 3), Conv(256, 256, 3),Conv(256,x,1)) for x in ch)  # reg_conf stem conv
#         #
...
#         self.m_stem = nn.ModuleList(Conv(x, x, 1) for x in ch)  # stem conv
#         self.m_cls = nn.ModuleList(
#             nn.Sequential(CA_Layer(x), InceptionD(x),nn.Conv2d(x, self.na * self.nc,1)) for x in ch)  # cls conv
#         # self.m_cls = nn.ModuleList(
#         #     nn.Sequential(Conv(x,256,1),CA_Layer(256), Conv(256, 256, 3),Conv(256, 256, 3),Conv(256,x,1),nn.Conv2d(x, self.na * self.nc, 1)) for x in ch)  # cls conv
#
#         self.m_reg_conf = nn.ModuleList(
#             nn.Sequential(SA_Layer(x),InceptionD(x))for x in ch)  # reg_conf stem conv
#         # self.m_reg_conf = nn.ModuleList(nn.Sequential(Conv(x,256,1),SA_Layer(256), Conv(256, 256, 3), Conv(256, 256, 3),Conv(256,x,1)) for x in ch)  # reg_conf stem conv
#         #
#         self.m_reg = nn.ModuleList(nn.Conv2d(x, self.na * 4, 1) for x in ch)  # reg conv
#         self.m_conf = nn.ModuleList(nn.Conv2d(x, self.na * 1, 1) for x in ch)  # conf conv
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, IDetect_Decoupled_Att):
            s = 256  # 2x min stride
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IDetect_Decoupled) or isinstance(m, TSCODE_Detect) or isinstance(m, IDetect_Decoupled_Att) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IDetect_Decoupled,TSCODE_Detect,IDetect_Decoupled_Att, IAuxDetect, IBin))
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        elif isinstance(m, IDetect_Decoupled_Att):
            for mi, s in zip(m.m_conf, m.stride):  # from
                b = mi.bias.view(m.na, -1)  # conv.bias(255) to (3,85)
                b.data += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
                mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d,Conv,gnconv,M_Conv,Ghost_Conv,Dilate_Conv,Dila_Conv,Deform_Conv2d, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC,CAM,CoordAtt,
                 SPP, SPPF,ASPPCSPC,SPPCSPC, GhostSPPCSPC, MixConv2d, Focus, Stem, GhostStem, CrossConv,
                 ...
                 SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC ,RFP_ASPP,ScConv,Inception,InceptionD,InceptionE]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [DownC,ASPPCSPC,SPPCSPC, GhostSPPCSPC,
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IDetect_Decoupled,TSCODE_Detect, IDetect_Decoupled_Att,IAuxDetect, IBin, IKeypoint]:
